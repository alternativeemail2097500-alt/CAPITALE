...
const diffAllBtn = document.getElementById("diffAllBtn");
...
const capitalPlate = document.querySelector(".capital-plate");
const continentTagEl = document.getElementById("continent-tag");
const tilesRowEl = document.getElementById("tiles-row");
const compassEl = document.getElementById("compass");
const compassNeedleEl = document.getElementById("compass-needle");
const closestPillEl = document.getElementById("closest-pill");
...
diffAllBtn.addEventListener("click", () => {
  difficulty = "all";
  diffAllBtn.classList.add("active");
  diffEasyBtn.classList.remove("active");
  socket.emit("set-difficulty", { level: difficulty });
});
diffEasyBtn.addEventListener("click", () => {
  difficulty = "easy";
  diffEasyBtn.classList.add("active");
  diffAllBtn.classList.remove("active");
  socket.emit("set-difficulty", { level: difficulty });
});
...
// ---------- Round lifecycle ----------
socket.on("round-start", ({ continent, tiles, roundSeconds, answer }) => {
  continentTagEl.textContent = continent || "—";
  renderTiles(tiles);
  closestPillEl.style.display = "none";
  compassEl.style.display = "none";
  capitalPlate.classList.remove("revealed");
  hintTextEl.textContent = "Type the country or capital in chat to guess.";
...
socket.on("tiles-update", ({ tiles }) => {
  renderTiles(tiles);
});

socket.on("wrong-guess", ({ guessedName, distanceKm, direction, closestKm }) => {
  ...
  compassNeedleEl.style.setProperty("--needle-deg", `${directionToDegrees(direction)}deg`);

  closestPillEl.style.display = "block";
  closestPillEl.textContent = `Closest guess so far: ${closestKm.toLocaleString()} km`;
});
...
socket.on("round-end", ({ countryName, capitalName, continent, winner, points, leaderboard: lb }) => {
  stopCountdown();
  revealFullAnswer(countryName);
  capitalPlate.classList.add("revealed");
  setTimeout(() => capitalPlate.classList.remove("revealed"), 1200);
  ...
  hintTextEl.innerHTML = `<strong>${escapeHtml(countryName)}</strong> — capital: ${escapeHtml(capitalName)}, ${escapeHtml(continent)}.`;
  hintTextEl.classList.remove("flash");
...
function renderTiles(tiles) {
  if (!tiles) { tilesRowEl.textContent = ""; return; }
  tilesRowEl.innerHTML = tiles
    .split("")
    .map((ch) => (ch === "_" ? `<span class="tile-blank">_</span>` : escapeHtml(ch)))
    .join("");
}

function revealFullAnswer(countryName) {
  tilesRowEl.textContent = countryName;
}
...
function directionToDegrees(direction) {
  return COMPASS_DEGREES[direction] ?? 0;
}
...
function renderLeaderboard(list) {
  if (!list || list.length === 0) {
    tickerTrackEl.textContent = "Waiting for the first correct guess…";
  } else {
    const items = list
      .map((e, i) => `<span class="tk-rank">${i + 1}.</span>${escapeHtml(e.username)} — ${e.score}pt`)
      .join('<span class="tk-sep">•</span>');
    tickerTrackEl.innerHTML = `🏆 Top explorers ${items}`;
  }
...
// Lightweight celebration: a big winner-name banner over the capital plate
// plus a short confetti burst. Kept cheap on purpose (small fixed particle
// count, pure CSS transforms, nodes removed right after animating) so it
// stays smooth even on a lower-end Android phone mid-broadcast.
